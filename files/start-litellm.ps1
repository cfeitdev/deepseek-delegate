# Starts the local LiteLLM gateway (127.0.0.1:4000) and, for Anthropic-compatible
# providers, the header filter (127.0.0.1:4011), if they aren't already running.
# Called by the deepseek-delegate skill's delegate.ps1. If the filter is down, requests
# to an Anthropic-compatible provider fail closed instead of going out unfiltered.
...
# running after the caller exits. Provider keys come from the user-level environment.
